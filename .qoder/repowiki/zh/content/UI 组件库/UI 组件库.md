...
graph TB
subgraph "应用层"
PB["Playbook 入口<br/>src/app/playbook/page.tsx"]
REG["注册表<br/>src/app/playbook/registry.ts"]
end
subgraph "UI 组件层"
U["基础 UI 组件<br/>src/components/ui/*"]
N["节点业务组件<br/>src/components/ui/node/*"]
I["图标组件<br/>src/components/icons/*"]
end
PB --> REG
REG --> U
REG --> N
...
Button <.. IconButton : "组合"
TextField <.. SearchField : "组合"
SettingsGroup <.. SettingsRow : "组合"
Sidebar <.. NavItem : "组合"
TopBar <.. IconButton : "组合"
StageNode <.. StatusPill : "组合"
StageNode <.. ProgressBar : "组合"
ProjectCard <.. ContactSheetThumb : "组合"
```
...
- 事件
  - 切换回调
- 示例路径
  - [toggle.demo.tsx](file://src/components/ui/toggle.tsx)
...
```mermaid
graph LR
REG["注册表<br/>registry.ts"] --> BTN["Button"]
REG --> DIALOG["Dialog"]
REG --> ICON_BTN["IconButton"]
REG --> TOOLTIP["Tooltip"]
REG --> TOGGLE["Toggle"]
REG --> TF["TextField"]
REG --> TA["TextArea"]
...
REG --> SID["Sidebar"]
REG --> NAV["NavItem"]
REG --> SG["SettingsGroup"]
REG --> SR["SettingsRow"]
REG --> SEG["SegmentedControl"]
REG --> QSB["QueueStatusBar"]
REG --> EMPTY["EmptyState"]
REG --> RH["ResizeHandle"]
REG --> SP["StatusPill"]
REG --> TOAST["Toast"]
REG --> CTH["ContactSheetThumb"]
REG --> PC["ProjectCard"]
REG --> TT["TimelineTrack"]
REG --> SN["StageNode"]
REG --> AN["AudioNode"]
REG --> EN["ExportNode"]
REG --> SHN["ShotNode"]
...
participant Page as "Playbook 页面"
participant Reg as "注册表"
participant Comp as "组件"
App->>Page : 加载 Playbook
Page->>Reg : 导入组件集合
Reg-->>Page : 返回已注册组件
Page->>Comp : 渲染组件与演示
```
...
## 结论
本 UI 组件库以清晰的分层与组合模式构建，覆盖从基础到业务的完整能力。通过注册表统一管理、主题变量驱动与完善的可访问性支持，确保一致的设计体验与良好的扩展性。建议在新增组件时同步完善 demo、测试与文档，以保持生态健康与可维护性。