...
## 更新摘要
**变更内容**   
- 新增六阶段输入契约规范，明确每个阶段的输入输出数据格式和验证规则
- 完善阶段执行器数据流要求，强化上下文传递和状态管理机制
- 增强类型定义和接口约束，提升系统健壮性和可维护性
- 优化错误处理和重试机制，提供更详细的诊断信息
...
subgraph "AI 集成"
L["Direct/Fabricate/Assemble/Finalize 提示模板"]
M["工具: write-artifact/validate-shot-plan/check-determinism"]
end
A --> C
B --> C
C --> D
...
- 内部依赖
  ...
  - **新增** StageRunner 依赖输入契约验证器和数据流管理器
...
## 结论
导演系统以 Pipeline 为核心编排器，结合 StageRunner、SessionStore 与 RuntimeRepository 形成稳定可扩展的工作流引擎。**新增的六阶段输入契约规范和阶段执行器数据流要求**进一步提升了系统的健壮性和可维护性。通过明确的阶段契约、幂等产物与健壮的错误/重试机制，系统能够可靠地驱动复杂的多阶段制作流程，并与 AI 服务无缝集成。未来可在可观测性、弹性伸缩与更丰富的工具生态上持续演进。
