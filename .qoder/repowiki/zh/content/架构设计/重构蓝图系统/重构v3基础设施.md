# 重构v3基础设施

<cite>
**本文引用的文件**   
- [README.md](file://README.md)
- [AGENTS.md](file://AGENTS.md)
- [package.json](file://package.json)
- [drizzle.config.ts](file://drizzle.config.ts)
- [next.config.ts](file://next.config.ts)
- [vitest.config.ts](file://vitest.config.ts)
- [specs/2026-07-24-refactor-v3-architecture-spec.md](file://docs/specs/2026-07-24-refactor-v3-architecture-spec.md)
- [specs/2026-07-24-refactor-v3-product-spec.md](file://docs/specs/2026-07-24-refactor-v3-product-spec.md)
- [specs/2026-07-24-refactor-v3-task-breakdown.md](file://docs/specs/2026-07-24-refactor-v3-task-breakdown.md)
- [issues/refactor-v3/README.md](file://docs/issues/refactor-v3/README.md)
- [issues/refactor-v3/issue-n0-baseline-and-bleeding-fixes.md](file://docs/issues/refactor-v3/issue-n0-baseline-and-bleeding-fixes.md)
- [issues/refactor-v3/issue-n1-postgres-foundation-and-spikes.md](file://docs/issues/refactor-v3/issue-n1-postgres-foundation-and-spikes.md)
- [issues/refactor-v3/issue-n2-trigger-orchestration.md](file://docs/issues/refactor-v3/issue-n2-trigger-orchestration.md)
- [issues/refactor-v3/issue-n3-pi-agent-runtime.md](file://docs/issues/refactor-v3/issue-n3-pi-agent-runtime.md)
- [issues/refactor-v3/issue-n4-artifact-compiler-hyperframes.md](file://docs/issues/refactor-v3/issue-n4-artifact-compiler-hyperframes.md)
- [issues/refactor-v3/issue-n5-compose-closure.md](file://docs/issues/refactor-v3/issue-n5-compose-closure.md)
- [issues/refactor-v3/issue-n6-ui-truth-and-governance.md](file://docs/issues/refactor-v3/issue-n6-ui-truth-and-governance.md)
- [issues/refactor-v3/issue-n7-e2e-and-cutover.md](file://docs/issues/refactor-v3/issue-n7-e2e-and-cutover.md)
- [adr/0001-postgres-local-cloud-ready.md](file://docs/adr/0001-postgres-local-cloud-ready.md)
- [adr/0002-trigger-execution-boundary.md](file://docs/adr/0002-trigger-execution边界.md)
- [adr/0003-pi-agent-runtime.md](file://docs/adr/0003-pi-agent-runtime.md)
- [adr/0004-video-compiler-hyperframes.md](file://docs/adr/0004-video-compiler-hyperframes.md)
- [src/app/api/director/pipeline/route.ts](file://src/app/api/director/pipeline/route.ts)
- [src/app/api/director/stage/route.ts](file://src/app/api/director/stage/route.ts)
- [src/app/api/director/stream/[nodeId]/route.ts](file://src/app/api/director/stream/[nodeId]/route.ts)
- [src/app/api/render/route.ts](file://src/app/api/render/route.ts)
- [src/app/api/artifacts/[id]/route.ts](file://src/app/api/artifacts/[id]/route.ts)
- [src/features/director/index.ts](file://src/features/director/index.ts)
- [src/features/director/pipeline.ts](file://src/features/director/pipeline.ts)
- [src/features/director/stage-runner.ts](file://src/features/director/stage-runner.ts)
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/director/session-store.ts](file://src/features/director/session-store.ts)
- [src/features/director/queue-handler.ts](file://src/features/director/queue-handler.ts)
- [src/features/director/advance.ts](file://src/features/director/advance.ts)
- [src/features/director/stage-result.ts](file://src/features/director/stage-result.ts)
- [src/features/director/stage-effects.ts](file://src/features/director/stage-effects.ts)
- [src/features/director/stage-prompt.ts](file://src/features/director/stage-prompt.ts)
- [src/features/director/pi-session.ts](file://src/features/director/pi-session.ts)
- [src/features/render/renderer.ts](file://src/features/render/renderer.ts)
- [src/features/render/export-service.ts](file://src/features/render/export-service.ts)
- [src/features/render/repository.ts](file://src/features/render/repository.ts)
- [src/features/render/cache.ts](file://src/features/render/cache.ts)
- [src/lib/db/index.ts](file://src/lib/db/index.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)
- [docs/evidence/refactor-v3/n0-baseline.json](file://docs/evidence/refactor-v3/n0-baseline.json)
</cite>

## 更新摘要
**所做更改**
- 新增基线捕获与验证系统章节，涵盖完整的基线测试框架
- 添加工作流版本化能力说明，包括version.ts的实现细节
- 扩展N0任务完成证据部分，详细说明n0-baseline.json的作用
- 更新基线重测试机制，增强测试稳定性和可重复性

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [基线捕获与验证系统](#基线捕获与验证系统)
7. [工作流版本化能力](#工作流版本化能力)
8. [依赖关系分析](#依赖关系分析)
9. [性能考量](#性能考量)
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)
12. [附录](#附录)

## 简介
本文件面向"重构v3基础设施"的目标，系统性梳理CodeVideoCanvas在Next.js应用层、导演编排（Director）、渲染管线、数据持久化与测试体系等方面的现状与演进方向。文档基于仓库中的规范、ADR与任务分解，结合现有API路由与特性模块的实现，给出分层职责、数据流与控制流说明，并提供可操作的排障与优化建议。

**最新更新**：新增了完整的基线捕获与验证系统，支持工作流版本化和基线重测试功能，为v3重构提供了稳定的测试基础。

## 项目结构
- 应用层：Next.js App Router组织页面与API路由，按功能域划分到app/(app)、app/api等路径。
- 特性层：features下按领域拆分（director、render、audio、artifacts、ai、canvas、navigation等），强调高内聚低耦合。
- 基础设施：lib提供通用能力（db、storage、stream、queue等），scripts提供部署与数据库迁移工具。
- 文档：docs包含ADR、设计、规范、问题清单与计划，指导重构目标与验收标准。
- **新增**：scripts/verify目录包含基线捕获和验证脚本，docs/evidence存储基线证据文件。

```mermaid
graph TB
A["Next.js 应用<br/>app/(app), app/api"] --> B["特性层 features<br/>director, render, audio, artifacts, ai, canvas, navigation"]
B --> C["基础设施 lib<br/>db, storage, stream, queue"]
B --> D["外部服务/存储<br/>PostgreSQL, 对象存储, AI模型"]
E["脚本 scripts<br/>db-migrate, spikes, dev, verify"] --> C
F["文档 docs<br/>ADR, specs, issues, evidence"] --> A
F --> B
G["基线系统<br/>capture-v3-baseline.ts<br/>n0-baseline.json"] --> E
```

**图表来源** 
- [next.config.ts](file://next.config.ts)
- [package.json](file://package.json)
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)
- [docs/evidence/refactor-v3/n0-baseline.json](file://docs/evidence/refactor-v3/n0-baseline.json)

**章节来源**
- [README.md](file://README.md)
- [package.json](file://package.json)

## 核心组件
- 导演编排（Director）：负责阶段式流水线推进、会话状态管理、队列处理与结果提交。
- 渲染管线（Render）：封装帧序列、编码、导出、缓存与缩略图生成，提供对外API。
- 数据持久化（DB）：通过Drizzle配置与迁移脚本对接PostgreSQL，支撑本地与云端一致的数据访问。
- API路由：暴露导演、渲染、制品、设置等接口，作为前端与后端能力的统一入口。
- **新增**：基线验证系统：提供完整的基线捕获、存储和验证机制，确保测试的稳定性和可重复性。

**章节来源**
- [src/features/director/index.ts](file://src/features/director/index.ts)
- [src/features/render/index.ts](file://src/features/render/index.ts)
- [drizzle.config.ts](file://drizzle.config.ts)
- [src/app/api/director/pipeline/route.ts](file://src/app/api/director/pipeline/route.ts)
- [src/app/api/render/route.ts](file://src/app/api/render/route.ts)
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)

## 架构总览
v3基础设施以"事件驱动+阶段编排"为核心，将AI生成、视频编译、导出与UI状态治理解耦，形成清晰的边界与可观测性。

```mermaid
graph TB
subgraph "应用层"
UI["Next.js 页面/组件"]
API["API 路由<br/>director, render, artifacts, settings"]
end
subgraph "特性层"
DIR["Director 编排<br/>pipeline, stage-runner, advance"]
RND["Render 渲染<br/>renderer, export-service, cache"]
ART["Artifacts 制品<br/>service"]
AUD["Audio 音频<br/>repository, runtime-repository"]
AI["AI 适配<br/>gemini, stepfun, model-routing"]
end
subgraph "基础设施"
DB["PostgreSQL (Drizzle)"]
Q["队列/并发控制"]
ST["对象存储/文件系统"]
BASELINE["基线系统<br/>capture, verify, versioning"]
end
UI --> API
API --> DIR
API --> RND
API --> ART
DIR --> Q
DIR --> DB
RND --> ST
RND --> DB
ART --> DB
ART --> ST
AUD --> DB
AI --> |调用| 外部模型服务
BASELINE --> DIR
BASELINE --> RND
BASELINE --> ART
```

**图表来源** 
- [src/app/api/director/pipeline/route.ts](file://src/app/api/director/pipeline/route.ts)
- [src/app/api/render/route.ts](file://src/app/api/render/route.ts)
- [src/features/director/pipeline.ts](file://src/features/director/pipeline.ts)
- [src/features/render/renderer.ts](file://src/features/render/renderer.ts)
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)

## 详细组件分析

### 导演编排（Director）
- 职责：维护会话与运行时上下文，解析阶段定义，调度执行器，处理副作用与结果合并，推进流水线。
- 关键流程：接收请求→校验输入→加载会话→选择阶段→执行阶段→提交结果→推进下一步。
- 错误处理：阶段失败回滚或重试，异常上报与日志记录，状态一致性保障。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "API路由"
participant Pipeline as "Pipeline"
participant Runner as "StageRunner"
participant Repo as "RuntimeRepository"
participant Store as "SessionStore"
Client->>API : "POST /api/director/pipeline"
API->>Pipeline : "启动/继续流水线"
Pipeline->>Store : "读取/更新会话"
Pipeline->>Repo : "获取运行时上下文"
Pipeline->>Runner : "执行当前阶段"
Runner-->>Pipeline : "阶段结果"
Pipeline->>Repo : "提交结果并推进状态"
Pipeline-->>API : "返回进度/结果"
API-->>Client : "响应"
```

**图表来源** 
- [src/app/api/director/pipeline/route.ts](file://src/app/api/director/pipeline/route.ts)
- [src/features/director/pipeline.ts](file://src/features/director/pipeline.ts)
- [src/features/director/stage-runner.ts](file://src/features/director/stage-runner.ts)
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/director/session-store.ts](file://src/features/director/session-store.ts)

**章节来源**
- [src/features/director/pipeline.ts](file://src/features/director/pipeline.ts)
- [src/features/director/stage-runner.ts](file://src/features/director/stage-runner.ts)
- [src/features/director/advance.ts](file://src/features/director/advance.ts)
- [src/features/director/queue-handler.ts](file://src/features/director/queue-handler.ts)
- [src/features/director/stage-result.ts](file://src/features/director/stage-result.ts)
- [src/features/director/stage-effects.ts](file://src/features/director/stage-effects.ts)
- [src/features/director/stage-prompt.ts](file://src/features/director/stage-prompt.ts)
- [src/features/director/pi-session.ts](file://src/features/director/pi-session.ts)

### 渲染管线（Render）
- 职责：帧序列管理、编码导出、缓存策略、缩略图生成与QA检查。
- 关键流程：构建帧序列→编码→写入存储→生成缩略图→更新制品元数据。
- 错误处理：编码失败重试、缓存失效清理、IO异常恢复。

```mermaid
flowchart TD
Start(["开始"]) --> BuildSeq["构建帧序列"]
BuildSeq --> Encode["编码/转码"]
Encode --> CacheCheck{"缓存命中?"}
CacheCheck --> |是| ReturnCache["返回缓存结果"]
CacheCheck --> |否| WriteStorage["写入存储"]
WriteStorage --> Thumb["生成缩略图"]
Thumb --> UpdateMeta["更新制品元数据"]
UpdateMeta --> End(["结束"])
ReturnCache --> End
```

**图表来源** 
- [src/features/render/renderer.ts](file://src/features/render/renderer.ts)
- [src/features/render/export-service.ts](file://src/features/render/export-service.ts)
- [src/features/render/cache.ts](file://src/features/render/cache.ts)
- [src/features/render/repository.ts](file://src/features/render/repository.ts)

**章节来源**
- [src/features/render/renderer.ts](file://src/features/render/renderer.ts)
- [src/features/render/export-service.ts](file://src/features/render/export-service.ts)
- [src/features/render/cache.ts](file://src/features/render/cache.ts)
- [src/features/render/repository.ts](file://src/features/render/repository.ts)

### 制品与存储（Artifacts & Storage）
- 职责：制品生命周期管理、元数据持久化、与对象存储的适配器抽象。
- 关键点：幂等写入、版本化、跨环境一致性。

**章节来源**
- [src/features/artifacts/service.ts](file://src/features/artifacts/service.ts)
- [src/app/api/artifacts/[id]/route.ts](file://src/app/api/artifacts/[id]/route.ts)

### 数据库与迁移（DB & Migration）
- 职责：使用Drizzle进行类型安全的ORM操作，提供本地与云端一致的数据库访问；脚本执行迁移。
- 关键点：连接池、事务、索引优化、迁移回滚策略。

**章节来源**
- [drizzle.config.ts](file://drizzle.config.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [src/lib/db/index.ts](file://src/lib/db/index.ts)

## 基线捕获与验证系统

### 概述
v3基础设施新增了完整的基线捕获与验证系统，为重构过程提供稳定的测试基础和回归检测能力。该系统包括基线数据捕获、存储管理、版本控制和自动化验证等功能。

### 核心组件

#### 基线捕获脚本
- **capture-v3-baseline.ts**：主捕获脚本，负责收集系统状态、测试结果和关键指标
- 支持多种数据源：API响应、数据库状态、文件系统快照
- 提供增量捕获和全量捕获两种模式

#### 基线数据存储
- **n0-baseline.json**：N0任务的基线数据文件，包含初始状态的完整快照
- 结构化JSON格式，便于版本控制和差异比较
- 包含时间戳、版本号和环境信息

#### 验证机制
- 自动对比当前状态与基线数据的差异
- 支持选择性验证特定组件或功能
- 提供详细的差异报告和修复建议

```mermaid
flowchart TD
Capture["基线捕获"] --> Validate["数据验证"]
Validate --> Store["存储管理"]
Store --> Version["版本控制"]
Version --> Compare["差异比较"]
Compare --> Report["报告生成"]
Report --> Fix["修复建议"]
Fix --> Revalidate["重新验证"]
Revalidate --> Capture
```

**图表来源** 
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)
- [docs/evidence/refactor-v3/n0-baseline.json](file://docs/evidence/refactor-v3/n0-baseline.json)

### N0任务基线证据
N0任务作为v3重构的基础任务，其基线数据包含了系统初始状态的完整快照：

- **系统配置快照**：数据库结构、环境变量、依赖版本
- **API接口基线**：所有端点的响应格式和默认值
- **组件状态基线**：关键组件的初始状态和配置
- **测试用例基线**：核心功能的预期行为和输出

**章节来源**
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)
- [docs/evidence/refactor-v3/n0-baseline.json](file://docs/evidence/refactor-v3/n0-baseline.json)
- [issues/refactor-v3/issue-n0-baseline-and-bleeding-fixes.md](file://docs/issues/refactor-v3/issue-n0-baseline-and-bleeding-fixes.md)

## 工作流版本化能力

### 版本管理架构
v3基础设施引入了完善的工作流版本化机制，支持对重构过程中的各个阶段进行精确的版本控制和追溯。

#### 版本数据结构
- **语义化版本**：遵循MAJOR.MINOR.PATCH格式
- **变更追踪**：记录每次变更的影响范围和修改内容
- **依赖关系**：明确各版本间的依赖和兼容性要求

#### 版本控制流程
1. **版本创建**：在关键里程碑创建新版本
2. **变更标记**：标记每个变更所属的版本
3. **兼容性检查**：验证版本间的向后兼容性
4. **发布管理**：管理版本的发布和回滚策略

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant Version as "版本管理器"
participant Baseline as "基线系统"
participant Test as "测试套件"
Dev->>Version : "创建新版本"
Version->>Baseline : "生成基线快照"
Baseline->>Test : "运行基准测试"
Test-->>Version : "测试结果"
Version-->>Dev : "版本状态报告"
```

**图表来源** 
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)
- [docs/evidence/refactor-v3/n0-baseline.json](file://docs/evidence/refactor-v3/n0-baseline.json)

### 版本回溯与恢复
- **快速回滚**：支持一键恢复到指定版本
- **差异分析**：自动分析版本间的具体差异
- **影响评估**：评估版本变更对现有功能的影响
- **渐进式升级**：支持从旧版本逐步升级到新版本

**章节来源**
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)
- [docs/evidence/refactor-v3/n0-baseline.json](file://docs/evidence/refactor-v3/n0-baseline.json)

## 依赖关系分析
- 应用层依赖特性层API，特性层依赖基础设施（DB、存储、队列）。
- Director与Render之间通过制品与运行时状态解耦，避免紧耦合。
- AI适配与模型服务通过适配器模式隔离，便于扩展与替换。
- **新增**：基线系统与所有核心组件集成，提供统一的测试和验证接口。

```mermaid
graph LR
API["API路由"] --> DIR["Director"]
API --> RND["Render"]
DIR --> DB["PostgreSQL"]
DIR --> Q["队列"]
RND --> ST["对象存储"]
RND --> DB
ART["Artifacts"] --> DB
ART --> ST
AI["AI适配"] --> Ext["外部模型服务"]
BASELINE["基线系统"] --> DIR
BASELINE --> RND
BASELINE --> ART
BASELINE --> DB
```

**图表来源** 
- [src/app/api/director/pipeline/route.ts](file://src/app/api/director/pipeline/route.ts)
- [src/app/api/render/route.ts](file://src/app/api/render/route.ts)
- [src/features/director/runtime-repository.ts](file://src/features/director/runtime-repository.ts)
- [src/features/render/repository.ts](file://src/features/render/repository.ts)
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)

**章节来源**
- [package.json](file://package.json)
- [next.config.ts](file://next.config.ts)

## 性能考量
- 并发与限流：队列处理器需限制并发度，避免资源争用与下游服务过载。
- 缓存策略：渲染缓存应区分粒度（帧级、片段级、成品级），提高命中率。
- I/O优化：大文件分块读写、异步I/O、压缩传输。
- 数据库：合理索引、分页查询、连接池调优。
- **新增**：基线系统性能优化：增量捕获、并行验证、内存管理。

## 故障排查指南
- 导演编排失败：检查阶段执行日志、会话状态一致性、队列积压情况。
- 渲染失败：查看编码日志、缓存失效原因、存储权限与配额。
- 数据库问题：确认连接配置、迁移状态、事务锁与慢查询。
- 外部服务：验证模型服务可用性、密钥配置与速率限制。
- **新增**：基线验证失败：检查基线文件完整性、版本兼容性、环境差异。

**章节来源**
- [src/features/director/queue-handler.ts](file://src/features/director/queue-handler.ts)
- [src/features/render/renderer.ts](file://src/features/render/renderer.ts)
- [scripts/setup/db-migrate.ts](file://scripts/setup/db-migrate.ts)
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)

## 结论
v3基础设施重构以清晰的分层与边界为基础，强化导演编排与渲染管线的可观测性与可扩展性，配合稳定的数据持久化与测试体系，为后续AI能力集成与产品迭代提供坚实基础。**新增的基线捕获与验证系统以及工作流版本化能力，为重构过程提供了可靠的测试基础和版本管理能力**。建议优先完成基线修复、数据库基础与触发编排，再逐步推进PI代理运行时、超帧编译器与UI治理，最终通过E2E与切换策略确保平滑过渡。

## 附录
- 规范与决策：参考docs/specs与docs/adr下的架构与产品规范、技术决策记录。
- 任务分解：依据docs/issues/refactor-v3下的问题清单逐项推进。
- 测试策略：使用Vitest进行单元与集成测试，覆盖关键路径与边界条件。
- **新增**：基线管理：使用capture-v3-baseline.ts进行基线捕获，通过n0-baseline.json管理基线数据。

**章节来源**
- [specs/2026-07-24-refactor-v3-architecture-spec.md](file://docs/specs/2026-07-24-refactor-v3-architecture-spec.md)
- [specs/2026-07-24-refactor-v3-product-spec.md](file://docs/specs/2026-07-24-refactor-v3-product-spec.md)
- [specs/2026-07-24-refactor-v3-task-breakdown.md](file://docs/specs/2026-07-24-refactor-v3-task-breakdown.md)
- [issues/refactor-v3/README.md](file://docs/issues/refactor-v3/README.md)
- [vitest.config.ts](file://vitest.config.ts)
- [scripts/verify/capture-v3-baseline.ts](file://scripts/verify/capture-v3-baseline.ts)
- [docs/evidence/refactor-v3/n0-baseline.json](file://docs/evidence/refactor-v3/n0-baseline.json)