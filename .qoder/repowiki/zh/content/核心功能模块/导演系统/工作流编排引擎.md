...
## 更新摘要
**变更内容**   
- 导演系统输入契约完善：resolveDirectorInput函数支持按nodeType分支处理不同类型节点输入
- 角色感知构建器重构：assemble和finalize函数拆分为独立的角色感知构建器
- FINALIZE阶段增强：增加MP4缺失时的早期失败处理机制
- 工作流编排引擎架构优化：提升输入验证与错误处理的健壮性
...
**更新** 本次更新重点反映了导演系统输入契约的完善，包括增强的节点类型处理、角色感知构建器重构以及FINALIZE阶段的早期失败处理机制。
...
**更新** 新增角色感知构建器组件，专门处理assemble和finalize阶段的特定业务逻辑，提升代码的可维护性和扩展性。
...
participant InputResolver as "输入解析器"
...
Pipeline->>InputResolver : "解析节点输入(nodeType分支)"
InputResolver-->>Pipeline : "标准化输入数据"
...
**更新** 增强了输入解析机制，现在支持按nodeType分支处理不同类型的节点输入，提供更灵活的输入契约。
...
**更新** 集成角色感知构建器，在assemble和finalize阶段使用专门的构建器处理业务逻辑，提升代码的可维护性。
...
**更新** 增强了错误处理机制，特别是在FINALIZE阶段增加了MP4缺失时的早期失败处理，提供更明确的错误反馈。
...
### 输入解析器与角色感知构建器
- 输入解析器
  - resolveDirectorInput函数现在支持按nodeType分支处理不同类型节点输入。
  - 提供统一的输入契约，确保不同节点类型的输入格式一致性。
- 角色感知构建器
  - assemble构建器：专门处理组装阶段的业务逻辑和数据整合。
  - finalize构建器：专门处理最终化阶段的输出整理和验证。
  - 提升代码的可维护性和扩展性，遵循单一职责原则。

**新增** 这是本次更新的核心改进，通过角色感知构建器重构了assemble和finalize函数的处理逻辑。

```mermaid
flowchart TD
Input["原始节点输入"] --> Resolver["输入解析器<br/>按nodeType分支处理"]
Resolver --> Assembler["Assemble构建器<br/>组装阶段处理"]
Resolver --> Finalizer["Finalize构建器<br/>最终化阶段处理"]
Assembler --> Validator["输入验证"]
Finalizer --> MP4Check["MP4文件检查"]
Validator --> Processed["标准化输入"]
MP4Check --> |存在| Processed
MP4Check --> |缺失| EarlyFail["早期失败处理"]
Processed --> NextStage["下一阶段处理"]
EarlyFail --> Error["错误上报"]
```
...
**更新** 新增了节点类型定义和输入契约接口，支持更丰富的节点类型处理和角色感知构建器的类型约束。
...
**更新** 新增输入解析器和角色感知构建器的依赖关系，提升了系统的模块化程度。
...
Pipeline --> InputResolver["输入解析器"]
Runner --> Result["StageResult"]
Runner --> Prompt["StagePrompt"]
Runner --> Assembler["Assemble构建器"]
Runner --> Finalizer["Finalize构建器"]
```
...
**更新** 角色感知构建器的引入减少了代码重复，提升了整体执行效率；早期失败处理机制避免了不必要的资源消耗。
...
- 常见问题
  ...
  - 输入解析错误：检查nodeType字段和输入数据格式是否符合新的输入契约。
  - MP4文件缺失：在FINALIZE阶段检查MP4文件是否存在，关注早期失败处理日志。
- 诊断步骤
  - 启用详细日志与追踪 ID，关联请求链路。
  - 使用运行时仓库查询阶段进度与中间产物，验证数据一致性。
  - 回放提示模板与输入数据，复现问题并回归测试。
  - 检查输入解析器的nodeType分支处理逻辑是否正确。
  - 验证角色感知构建器的业务逻辑执行情况。
- 恢复策略
  - 对可重试错误应用指数退避；对不可恢复错误快速失败并通知。
  - 支持断点续跑，从最近成功阶段继续执行。
  - 对于MP4缺失等确定性错误，立即终止工作流并返回明确错误信息。

**更新** 新增了输入解析和角色感知构建器相关的故障排查指导，特别强调了FINALIZE阶段MP4缺失的处理。
...
**更新** 本次更新通过完善输入契约、引入角色感知构建器和增强错误处理机制，进一步提升了系统的健壮性和可维护性。新的架构设计使得系统能够更好地处理复杂的节点类型和业务流程，同时保持了良好的性能和扩展性。
...
- 输入解析示例路径
  - 参考 pipeline.ts 中的 resolveDirectorInput 函数，了解按nodeType分支处理不同节点输入的机制。
- 角色感知构建器示例路径
  - 参考 stage-runner.ts 中的 AssembleBuilder 和 FinalizeBuilder 类，了解角色感知的构建器实现。

**更新** 新增了输入解析和角色感知构建器的相关示例路径，帮助开发者更好地理解和使用新的架构特性。
