...
| `ASSEMBLE`（`score`/`shot-sfx`/`shot-subtitle`） | `score`：`director-direct` 的 `styleBible` + `director-ingest` 的 `audioAllocation` + 全部 `shot-script` 的 `director-shot-spec` + 全部 `shot-codegen` 的 `render-mp4` 路径；`shot-sfx`/`shot-subtitle`：本通道 `director-shot-spec` + `audioAllocation` 对应条目（`shot-sfx` 另取 `render-mp4` 路径与 `styleBible`；`shot-subtitle` 另按 `audioUnitId` 取 `scriptUnit`） | `score → { styleBible, shotPlan, audioAllocation, renderedArtifactKeys }`；`shot-sfx → { shot, shotAllocation, renderedArtifactKey, styleBible }`；`shot-subtitle → { shot, scriptUnit, shotAllocation }` | ✅ 已实现（`resolveAssembleInput` 按 `nodeType` 分发；issue-01） |
| `FINALIZE`（`export`/`shot-qa`） | `export`：全部 `director-shot-spec` 聚合 + 最新 `final-mp4` 路径 + 全部 `shot-qa` 的 `director-finalize` 文本（缺失跳过）；`shot-qa`：本通道 `director-shot-spec` + `render-mp4` 路径 + `audioAllocation` 对应条目 | `export → { shotPlan, draftArtifactKey, qaFindings }`；`shot-qa → { shot, renderedArtifactKey, shotAllocation }` | ✅ 已实现（`resolveFinalizeInput` 按 `nodeType` 分发；`export` 缺 `final-mp4` 时前置抛可读错误；issue-01） |
